...
## payment details
inhouse = payment[payment["Current B2B online payment solutions"] ==
                  "Inhouse BNPL"]["Share of total B2B online volume"].iloc[0]
inhouse = float(inhouse.strip("%")) / 100
inhouse_cost = payment[payment["Current B2B online payment solutions"] ==
                       "Inhouse BNPL"]["Assumed costs"].iloc[0]
inhouse_cost = float(inhouse_cost.strip("%")) / 100
external = payment[payment["Current B2B online payment solutions"] ==
                   "External BNPL"]["Share of total B2B online volume"].iloc[0]
external = float(external.strip("%")) / 100
external_cost = payment[payment["Current B2B online payment solutions"] ==
                        "External BNPL"]["Assumed costs"].iloc[0]
external_cost = float(external_cost.strip("%")) / 100
credit_card = payment[
    payment["Current B2B online payment solutions"] ==
    "Credit Card"]["Share of total B2B online volume"].iloc[0]
credit_card = float(credit_card.strip("%")) / 100
credit_cost = payment[payment["Current B2B online payment solutions"] ==
                      "Credit Card"]["Assumed costs"].iloc[0]
credit_cost = float(credit_cost.strip("%")) / 100
debit_card = payment[payment["Current B2B online payment solutions"] ==
                     "Debit Card"]["Share of total B2B online volume"].iloc[0]
debit_card = float(debit_card.strip("%")) / 100
debit_cost = payment[payment["Current B2B online payment solutions"] ==
                     "Debit Card"]["Assumed costs"].iloc[0]
debit_cost = float(debit_cost.strip("%")) / 100
paypal = payment[payment["Current B2B online payment solutions"] ==
                 "Paypal"]["Share of total B2B online volume"].iloc[0]
paypal = float(paypal.strip("%")) / 100
paypal_cost = payment[payment["Current B2B online payment solutions"] ==
                      "Paypal"]["Assumed costs"].iloc[0]
paypal_cost = float(paypal_cost.strip("%")) / 100.0
other = payment[payment["Current B2B online payment solutions"] ==
                "Other"]["Share of total B2B online volume"].iloc[0]
other = float(other.strip("%")) / 100
other_cost = payment[payment["Current B2B online payment solutions"] ==
                     "Other"]["Assumed costs"].iloc[0]
other_cost = float(other_cost.strip("%")) / 100

## financial details
revenue = financial[financial["Metric"] ==
                    "B2B revenues p.a. Online:"]["value"].iloc[0]
revenue = float(str(revenue).replace(",", ""))
gross_profit = financial[financial["Metric"] ==
                         "Gross profit margin:"]["value"].iloc[0]
gross_profit = float(gross_profit.strip("%")) / 100
avg_basket_size = financial[financial["Metric"] ==
                            "Average Basket Size:"]["value"].iloc[0]
avg_basket_size = float(avg_basket_size)
avg_acceptance_rate = financial[financial["Metric"] ==
                                "Average Acceptance Rate:"]["value"].iloc[0]
...
transaction_fee = pricing[pricing["Metric"] ==
                          "Transaction Fee:"]["value"].iloc[0]
...
acceptance_rate_abs_chg = (np.nan if total_bnpl == 0 else
                           acceptance_rate_w_billie - acceptance_rate_wo_bilie)
conversion_rate_wo_billie = np.nan if total_bnpl > 0 else 1 - cart_abandonment_rate
conversion_rate_w_billie = (np.nan if total_bnpl > 0 else min(
    conversion_rate_wo_billie * (1 + uplift_conversion_rate), 1))
conversion_rate_absolute_chg = (np.nan if total_bnpl > 0 else
                                conversion_rate_w_billie -
                                conversion_rate_wo_billie)
conversion_rate_relative_chg = np.nan if total_bnpl > 0 else uplift_conversion_rate

revenue_chg_basket_size = (max_share if total_bnpl > 0 else
                           (max_share - total_bnpl) * uplift_basket_size *
                           revenue)

revenue_chg_acceptance_rate = ((revenue * total_bnpl / acceptance_rate_wo_bilie
                                if acceptance_rate_wo_bilie != 0 else np.nan) *
                               coalesce(acceptance_rate_abs_chg, 0) *
                               (1 - coalesce(buyer_not_accepted_bnpl, 0)))

revenue_chg_conversion_rate = revenue * (1 + uplift_conversion_rate) - revenue

revenue_abs_chg = (coalesce(revenue_chg_conversion_rate, 0) +
...
total_amount_wo_billie = (inhouse_amount_wo_billie +
                          external_amount_wo_billie +
                          credit_card_amount_wo_bilie +
                          debit_card_amount_wo_billie +
                          paypal_amount_wo_billie + other_amount_wo_billie)
...
credit_card_share_w_bilie = ((1 - billie_share) * credit_card /
                             total_non_bnpl if total_non_bnpl != 0 else 0)
debit_card_share_w_billie = ((1 - billie_share) * debit_card /
                             total_non_bnpl if total_non_bnpl != 0 else 0)
paypal_share_w_billie = ((1 - billie_share) * paypal /
                         total_non_bnpl if total_non_bnpl != 0 else 0)
other_share_w_billie = ((1 - billie_share) * other /
                        total_non_bnpl if total_non_bnpl != 0 else 0)
...
total_amount_w_billie = (billie_amount + inhouse_amount_w_billie +
                         external_amount_w_billie +
                         credit_card_amount_w_bilie +
                         debit_card_amount_w_billie + paypal_amount_w_billie +
...
total_cost_amnt_wo_billie = (inhouse_cost_amnt_wo_billie +
...
total_cost_amnt_w_billie = (billie_cost_amnt + inhouse_cost_amnt_w_billie +
...
wavg_cost_wo_billie = (
    (inhouse_amount_wo_billie * inhouse_cost + external_amount_wo_billie *
     external_cost + credit_card_amount_wo_bilie * credit_cost +
     debit_card_amount_wo_billie * debit_cost + paypal_amount_wo_billie *
     paypal_cost + other_amount_wo_billie * other_cost) /
    total_amount_wo_billie if total_amount_wo_billie != 0 else 0)

wavg_cost_w_billie = (billie_amount * cost_billie +
...
total_gross_profit_wo_billie = (inhouse_gross_profit_wo_billie +
...
other_gross_profit_w_billie = other_amount_w_billie * (
    gross_profit - (other_cost - wavg_cost_wo_billie))
total_gross_profit_w_billie = (billie_gross_profit_w_billie +
...
amount_rel_chg = (total_amount_w_billie -
                  total_amount_wo_billie) / total_amount_w_billie

cost_rel_chg = (total_cost_amnt_w_billie -
                total_cost_amnt_wo_billie) / total_cost_amnt_w_billie
